import { suite, test, Assert } from '../../../main/typescript/ts/testing/qunit';
...
export default suite('ts/iterator', (self) => {
  ...
  test('.iterate()', (assert) => {
    const odds = iterate(1, function(v) { return v + 2; });
  ...
  test('.concat()', (assert) => {
    const iter = concat(range(1, 4), range(4, 8), range(8, 10));
