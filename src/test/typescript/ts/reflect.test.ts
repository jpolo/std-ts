...
export default suite("ts/reflect", () => {
  ...
  test(".apply()", (assert) => {
    ...
    let returnValue = reflect.apply(fn, obj, [ "foo", "bar" ]);
  ...
  test(".construct()", (assert) => {
    let childObj = reflect.construct(Child, [ "$0", "$1" ]);
  ...
  test(".ownKeys()", (assert) => {
    let keys = reflect.ownKeys(Child.prototype);
  ...
  test(".stringTag(o)", (assert) => {
    let stringTag = reflect.stringTag;
