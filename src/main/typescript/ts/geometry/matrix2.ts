...
function ArrayCreate(Constructor: Matrix2Constructor): Matrix2 {
  return new Constructor(4);
}
function ArrayCreateFrom(o: Matrix2) {
  return ArrayCreate(GetConstructor(o));
}
function ArrayCopy(src: Matrix2, dest: Matrix2) {
  if (src !== dest) {
    dest[0] = src[0];
    dest[1] = src[1];
    dest[2] = src[2];
...
function ArrayFill(a: Matrix2, v: number) {
  a[0] = v;
  a[1] = v;
  a[2] = v;
  a[3] = v;
