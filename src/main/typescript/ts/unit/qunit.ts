...
module qunit {
  ...
  //Util
  var __protoOf = reflect.getPrototypeOf;
  var __str = function (o: any) { return "" + o; };
  ...
  export class Assert {
    ...
    __assert__(isSuccess: boolean, message: string, position: IAssertionCallSite): boolean {
      ...
      assertions.push(
        new Assertion(isSuccess ? assertion.SUCCESS : assertion.FAILURE, this._testCase, message, position)
      )
      return isSuccess
    }
   ...
   * Default suite
   */
  var suiteDefault = unit.suiteDefault;
  ...
  export function suite(
    ...
    var suiteNew = new unit.TestSuite(name);
